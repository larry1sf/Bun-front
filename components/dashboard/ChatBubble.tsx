...
function ChatBubbleUser({ message, image_url }: { message?: string, image_url?: string }) {
    ...
    return (
        <div className="flex items-start flex-row-reverse space-x-4 space-x-reverse">
            <div className="w-8 h-8 rounded-full bg-slate-800 shrink-0 flex items-center justify-center text-xs font-bold text-slate-400 border border-slate-700">{user?.username?.toUpperCase().charAt(0)}</div>
